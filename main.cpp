...
void update() {
    ...
    if (col) {
        // Reset the position of the moving circle
        X = -R;
...
void drawBlast(int centerX, int centerY) {
    int blastRadius = 100;
    for (int x = -blastRadius; x <= blastRadius; x++) {
        for (int y = -blastRadius; y <= blastRadius; y++) {
            if (x * x + y * y <= blastRadius * blastRadius) {
                ...
                int alpha = std::max(0, 255 - static_cast<int>(distance * 2.5));
                SDL_SetRenderDrawColor(rend, 255, 255, 0, alpha);
                SDL_RenderDrawPoint(rend, centerX + x, centerY + y);
            }
        }
    }
